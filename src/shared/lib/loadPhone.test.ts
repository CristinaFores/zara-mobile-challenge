...
describe('Given loadPhoneDetail', () => {
  ...
  describe('When the phone id is malformed', () => {
    it('Then it calls notFound() without hitting the external API', async () => {
      ...
      mockGetPhoneById.mockRejectedValueOnce(new ProductNotFoundError())
      const { loadPhoneDetail } = await import('./loadPhone')

      await expect(loadPhoneDetail('foo/bar')).rejects.toThrow('NEXT_NOT_FOUND')
      expect(mockNotFound).toHaveBeenCalledTimes(1)
    })
  })
